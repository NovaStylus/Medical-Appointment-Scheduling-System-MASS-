...
<nav class="navbar sticky-top bg-primary py-1" data-bs-theme="dark">
    ...
    <span class="navbar-brand mb-0 h1 d-flex align-items-center ps-2">
     ...
     <div class="nav-links">
    <button class="btn btn-sm" onclick="window.location.href='rec_login.php'" style="border-radius: 10px; padding: 8px 16px; background-color: white; color: black; border: none; cursor: pointer;">
      Login here
    </button>
  </div>
</nav>
<?php
  ...
  $errors = [];
  $receptionistLimitReached = false;

  // Check if two receptionist accounts already exist
  $checkStmt = $conn->prepare("SELECT COUNT(*) FROM receptionist");
  $checkStmt->execute();
  ...
  if ($count >= 2) {
      $receptionistLimitReached = true;
  }

  // Handle form submission only if limit not reached
  if ($_SERVER["REQUEST_METHOD"] == "POST" && !$receptionistLimitReached) {
      ...
      if (empty($errors)) {
          ...
          $stmt = $conn->prepare("INSERT INTO receptionist (fullName, email, phone, password) VALUES (?, ?, ?, ?)");
  ...
  if ($receptionistLimitReached) {
      echo "<p class='error'>Only two receptionist accounts are allowed. You cannot create more.</p>";
  }
?>

<?php if (!$receptionistLimitReached): ?>
  <form method="post" action="rec_signup.php">
    <h3>Signup Form</h3>
