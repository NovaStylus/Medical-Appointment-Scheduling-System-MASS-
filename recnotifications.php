...
$receptionistId = $_SESSION['receptionist_id'] ?? null;
if (!$receptionistId) {
    die("Please login first.");
}

// Mark notifications as read
$stmtUpdate = $conn->prepare("UPDATE recnotifications SET is_read = 1 WHERE receptionist_id = ? AND is_read = 0");
$stmtUpdate->bind_param("i", $receptionistId);
$stmtUpdate->execute();
$stmtUpdate->close();

// Fetch notifications
$stmt = $conn->prepare("SELECT * FROM recnotifications WHERE receptionist_id = ? ORDER BY created_at DESC");
...
if ($result->num_rows === 0) {
    echo '<div class="empty">You have no notifications yet.</div>';
} else {
