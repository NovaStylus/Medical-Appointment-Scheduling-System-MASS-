...
$sql = "SELECT email FROM users WHERE otp = ? AND TIMESTAMPDIFF(SECOND, otp_generated_time, NOW()) <= 300";
...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if ($newPassword !== $retypePassword) {
        ...
        $update = $conn->prepare("UPDATE users SET password = ?, otp = NULL, otp_generated_time = NULL WHERE email = ?");
        $update->bind_param("ss", $hashedPassword, $email);

        if ($update->execute()) {
            unset($_SESSION['otp']);
            $success = "✅ Password has been successfully reset. You can now <a href='login.php'>login</a>.";
        } else {
